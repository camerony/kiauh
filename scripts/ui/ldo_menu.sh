  ...
  while true; do
    ...
    case "${action}" in
      1)
        ...
        ldoconfig "V01" "A0" "00" 120 120 120
        ldo_menu;;
      2)
        select_msg "V0.1-S1 Rev E"
        ldoconfig "V01" "E0" "00" 120 120 120
        ldo_menu;;
      3)
        select_msg "V0.2-S1 Rev A/A+"
        ldoconfig "V02" "A0" "00" 120 120 120
        ldo_menu;;
  ...
  while true; do
    ...
    case "${action}" in
      1)
        ...
        ldoconfig "V24" "B0" "00" 250 250 200
        ldo_menu;;
      2)
        select_msg "Rev A/B 300mm"
        ldoconfig "V24" "B0" "00" 300 300 250
        ldo_menu;;
      3)
        select_msg "Rev A/B 300mm"
        ldoconfig "V24" "B0" "00" 350 350 300
        ldo_menu;;
      4)
        select_msg "Rev C 250mm"
        ldoconfig "V24" "C0" "00" 250 250 200
        ldo_menu;;
      5)
        select_msg "Rev C 350mm"
        ldoconfig "V24" "C0" "00" 300 300 250
        ldo_menu;;
      6)
        select_msg "Rev C 350mm"
        ldoconfig "V24" "C0" "00" 350 350 300
        ldo_menu;;
      B|b)
        clear; ldo_menu; break;;
      *)
...
function ldovt_ui() {
  ...
  while true; do
    ...
    case "${action}" in
      1)
        select_msg "Rev A/B 250mm"
        ldoconfig "VT0" "B0" "00" 250 250 200
        ldo_menu;;
      2)
        select_msg "Rev A/B 300mm"
        ldoconfig "VT0" "B0" "00" 300 300 250
        ldo_menu;;
      3)
        select_msg "Rev C 250mm"
        ldoconfig "VT0" "C0" "00" 250 250 200
        ldo_menu;;
      4)
        select_msg "Rev C 350mm"
        ldoconfig "VT0" "C0" "00" 300 300 250
        ldo_menu;;
      B|b)
        clear; ldo_menu; break;;
      *)
...
function download_ldo_configs() {
  ...
  ms_cfg_repo="https://github.com/camerony/LDOConfig.git"

  status_msg "Cloning LDOConfig ..."
  [[ -d "${HOME}/LDOConfig" ]] && rm -rf "${HOME}/LDOConfig"
  if git clone --recurse-submodules "${ms_cfg_repo}" "${HOME}/LDOConfig"; then
    ok_msg "Done!"
  else
  ...
  while true; do
    ...
    case "${yn}" in
      Y|y|Yes|yes|"")
        ...
        if ! sudo cp "${HOME}/LDOConfig/configs/${configfilename}" "${selected_printer_cfg}"; then
  ...
  while true; do
    ...
    case "${yn}" in
      Y|y|Yes|yes|"")
        ...
        if ! sudo cp "${HOME}/LDOConfig/configs/config.txt" "/boot/firmware/config.txt"; then
...
LDOCONFIG_DIR="${HOME}/LDOConfig"
function remove_ldoconfig() {
  [[ ! -d ${LDOCONFIG_DIR} ]] && return

  status_msg "Removing LDOConfig directory ..."
  rm -rf "${LDOCONFIG_DIR}" && ok_msg "Directory removed!"
}
